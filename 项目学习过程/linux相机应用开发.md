...
// 关键字段：
//   cap.driver[16]      驱动名，如 "uvcvideo"
//   cap.card[32]        设备名，如 "USB Camera"
//   cap.capabilities    能力位标志（重点看下面两个）
```

判断是否采集/流式：

```c
if (!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) { /* 不是采集设备 */ }
if (!(cap.capabilities & V4L2_CAP_STREAMING))     { /* 不支持流式 mmap 方式 */ }
```

### 4.2 `struct v4l2_format` —— 格式（含 `v4l2_pix_format`）

```c
struct v4l2_format fmt = {0};
fmt.type                = V4L2_BUF_TYPE_VIDEO_CAPTURE;  // 视频采集
fmt.fmt.pix.width       = 640;                          // 目标分辨率宽
fmt.fmt.pix.height      = 480;                          // 目标分辨率高
fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;            // 像素格式（见第 6 节）
fmt.fmt.pix.field       = V4L2_FIELD_NONE;              // 逐行，无隔行
ioctl(fd, VIDIOC_S_FMT, &fmt);

// ⚠️ 重要：S_FMT 是"协商"，驱动可能返回跟你请求不一样的实际值！
// 所以设置完后一定要读回：
//   fmt.fmt.pix.width / height  实际分辨率
//   fmt.fmt.pix.pixelformat     实际像素格式
//   fmt.fmt.pix.sizeimage       一帧数据的字节数（后面存文件/算缓冲要用）
```

### 4.3 `struct v4l2_requestbuffers` —— 申请缓冲

```c
struct v4l2_requestbuffers req = {0};
req.count  = 4;                             // 申请 4 个缓冲
req.type   = V4L2_BUF_TYPE_VIDEO_CAPTURE;
req.memory = V4L2_MEMORY_MMAP;              // 用 mmap 方式（推荐）
ioctl(fd, VIDIOC_REQBUFS, &req);
// req.count 会返回实际分配到的个数，后续循环用它
```

### 4.4 `struct v4l2_buffer` —— 单个缓冲的描述
...
// QUERYBUF：查询第 i 个缓冲的信息
ioctl(fd, VIDIOC_QUERYBUF, &buf);
//   buf.length         缓冲大小（mmap 长度用它）
//   buf.m.offset       偏移量（mmap 偏移用它）

// QBUF / DQBUF 都传同一个 buf，靠 index 区分
// DQBUF 返回后：
//   buf.index          出队的是哪个缓冲
//   buf.bytesused      这一帧实际有效字节数（!! 存文件/发送要用它，不是 buf.length !!）
//   buf.flags          帧状态（如 V4L2_BUF_FLAG_ERROR 表示帧出错）
```
